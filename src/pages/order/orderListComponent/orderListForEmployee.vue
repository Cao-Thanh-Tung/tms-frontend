...
async function getNumberOfOrder() {
    let num = 0;
    try {
        num += (await orderApi.getNumberOfOrderAssignToEmployee(myId)).data!;
    } catch (e: any) {
        console.log(e);
    }
...
const queryData = async (params: APIParams) => {
    try {
        ...
        return (await orderApi.getOrdersAssignToEmployee(myId, params.page! - 1, 6)).data;
    } catch (e) {
        console.log(e);
    }
    return null;
...
onMounted(() => {
    userxApi.getCustomerForEmployee(myId, 0, 5).then((res) => {
...
const fetchUser = debounce(value => {
    state.data = [];
    state.fetching = true;
    userxApi.searchCustomerAssignToEmployeeByFullName(myId, value, 0, 5).then((res) => {
        state.data = res.data.map(user => ({
            label: `${user.user?.firstName} ${user.user?.lastName}`,
            value: user.id!,
        }));
...
<template>
    ...
    <a-modal width="900px" v-if="editFormOpen" v-model:open="editFormOpen" title="Chỉnh sửa đơn hàng"
        ...
        <a-form v-else>
            ...
            <a-form-item label="Khung thời gian trả hàng">
                <a-range-picker :show-time="{ format: 'HH:mm' }" format="YYYY-MM-DD HH:mm"
                    :placeholder="['Start Time', 'End Time']" :value="editFormContent.rangeTo" />
            </a-form-item>
            <a-form-item label="Địa chỉ trả hàng">
                <address-form :initial-address-id="editFormContent.pickupAddressId!"
