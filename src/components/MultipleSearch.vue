<template>
  <div>
    <div
      v-for="(criterion, index) in searchCriteria"
      :key="index"
      class="criterion"
    >
      <a-select
        v-model:value="criterion.attributeName"
        class="criterion-select column"
        :disabled="true"
      >
        <a-select-option
          ...
          :key="field.name"
          :value="field.name"
          >{{ field.displayName }}</a-select-option
        >
      </a-select>
      ...
      <a-button @click="removeCriterion(index)" class="remove-button"
        >Xóa</a-button
      >
    </div>
    <div v-if="showCriterion" class="criterion">
      <div class="criterion-content">
        ...
        <a-select v-model:value="selectedOperator" class="criterion-select">
          <a-select-option value="EQUAL">Bằng</a-select-option>
          <a-select-option
            v-if="getSelectedFieldType(selectedField || 'unknown') !== 'string'"
            value="GREATER_THAN"
            >Lớn hơn</a-select-option
          >
          <a-select-option
      ...
      <div class="button-group">
        <a-button @click="showCriterion = false" class="cancel-button"
          >Hủy</a-button
        >
        <a-button @click="addAndHideCriterion" class="add-button"
          >Thêm</a-button
        >
      </div>
    </div>
    <a-input
      ...
      class="keyword-input"
    />
    <a-button @click="showCriterion = true" class="add-criterion-button"
      >Thêm trường tìm kiếm</a-button
    >
